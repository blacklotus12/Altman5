...
namespace Altman.Controls
{
    public class ConsoleBox : RichTextBox
    {
        ...
        void CmdBox_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            //不允许退格符移动到提示文字之前
            if (e.KeyCode == Keys.Back && IsCaretJustBeforePrompt())
            {
                e.Handled = true;
            }
            //不允许插入符号移动到提示文字之前
	        ...
	        if (e.KeyCode == Keys.Space && IsCaretJustBeforePrompt())
	        {
				e.Handled = true;
	        }
        ...
        private void ReplaceTextAtPrompt(string text)
        {
            ...
            {
                this.Select(this.Text.Length - charactersAfterPrompt, this.Text.Length);
        ...
        {
            return this.Text.Length - this.SelectionStart <= GetCurrentLine().Length;
        }
        /// <summary>
        /// 判断插入符号是否在可写位置
        /// </summary>
